  ...
  const updateEventMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const eventData = {
        ...
        scheduleItems: data.scheduleItems.map((item: ScheduleItem) => ({ time: item.time, activity: item.activity })),
        tags: data.tags,
      ...
      // Use the actual event ID from the loaded event data, not the URL param
    ...
    onSuccess: (updatedEvent) => {
      ...
      // Always use slug if available for better URLs
      const destination = updatedEvent?.slug || eventData?.slug || id
